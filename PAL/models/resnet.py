...
class Special_module(nn.Module):
    def __init__(self, drop_last_stride, modality_attention):
        ...
        special_module= resnet50(pretrained=True, drop_last_stride=drop_last_stride,
                           modality_attention=modality_attention)
        # avg pooling to global pooling
        #model_base.avgpool = nn.AdaptiveAvgPool2d((1, 1))   #源代码这句话要加上，不知道有没有用
...
class Shared_module_bh(nn.Module):
    def     __init__(self, drop_last_stride, modality_attention):
        ...
        model_sh_bh = resnet50(pretrained=True, drop_last_stride=drop_last_stride,
                           modality_attention=modality_attention)  #model_sh_fr  model_sh_bh
        # avg pooling to global pooling
        #model_base.avgpool = nn.AdaptiveAvgPool2d((1, 1))   #源代码这句话要加上，不知道有没有用
...
class special_att(nn.Module):
    def __init__(self, dim, r=16):
        ...
        self.IN = nn.InstanceNorm2d(dim, track_running_stats=True, affine=True)
...
class embed_net(nn.Module):
    def __init__(self, drop_last_stride, modality_attention, decompose=False):
        super(embed_net, self).__init__()

        self.shared_module_fr = Shared_module_fr(drop_last_stride=drop_last_stride, modality_attention=modality_attention)
        self.shared_module_bh = Shared_module_bh(drop_last_stride=drop_last_stride, modality_attention=modality_attention)

        self.special = Special_module(drop_last_stride=drop_last_stride, modality_attention=modality_attention)
    ...
    def forward(self, x):
        ...
        if self.decompose:
            ...
            x_sp, x_IN = self.special_att(x_sp_f)



            sp_f_pl = gem(x_sp_f).squeeze()  # Gem池化
            sp_f_pl = sp_f_pl.view(sp_f_pl.size(0), -1)  # Gem池化


            sp_pl = gem(x_sp).squeeze()  # Gem池化
            sp_pl = sp_pl.view(sp_pl.size(0), -1)  # Gem池化

            IN_pl = gem(x_IN).squeeze()  # Gem池化
            IN_pl = IN_pl.view(IN_pl.size(0), -1)  # Gem池化
            return x_sh, x_sp, sh_pl, sp_f_pl, sp_pl, IN_pl, x2, x3
        return sh_pl
