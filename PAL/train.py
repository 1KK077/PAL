    ...
    if cfg.optimizer == 'adam':
        if True == False:
            ...
            for name, param in model.named_parameters():
                ...
                elif 'classifier_new' in name:
                    other_params.append(param)

            # 定义优化器，给不同的参数组设置不同的权重衰减
            optimizer = optim.Adam([
                {'params': backbone_params, 'weight_decay': 5e-3},
                {'params': other_params, 'weight_decay': 5e-2},
            ], lr=cfg.lr)
            # import pdb
            # pdb.set_trace()
        ###########
...
if __name__ == '__main__':
    ...
    if cfg.sample_method == 'identity_uniform' or 'identity_random':
